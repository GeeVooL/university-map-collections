...
class OperationCountingObject {
 public:
  OperationCountingObject(int value_ = 0) : value(value_) {
    ++constructedObjects;
  }
  ...
  ~OperationCountingObject() { ++destroyedObjects; }
  ...
  static std::size_t constructedObjectsCount() { return constructedObjects; }

  static std::size_t destroyedObjectsCount() { return destroyedObjects; }

  static std::size_t copiedObjectsCount() { return copiedObjects; }

  static std::size_t movedObjectsCount() { return movedObjects; }

  static std::size_t assignedObjectsCount() { return assignedObjects; }
...
std::ostream& operator<<(std::ostream& out,
                         const OperationCountingObject& obj) {
  return out << '<' << static_cast<int>(obj) << '>';
}

struct Fixture {
  Fixture() { OperationCountingObject::resetCounters(); }
};
...
using TestedKeyTypes =
    boost::mpl::list<std::int32_t, std::uint64_t, OperationCountingObject>;
...
void thenMapContainsItems(const Map<K>& map,
  ...
  for (const auto& item : expected) {
    ...
    BOOST_REQUIRE_MESSAGE(it != end(map),
                          "Missing required item with key: " << item.first);
    BOOST_CHECK_MESSAGE(it->second == item.second,
                        "Wrong value in map for key: "
                            << item.first << " (expected: \"" << item.second
                            << "\" got: \"" << it->second << "\")");
  }
}
...
void thenConstructedObjectsCountWas(std::size_t count) {
  (void)count;
  // unable to check it (in a simple way) for all objects, hence template
  // specialization.
}
...
template <>
void thenConstructedObjectsCountWas<OperationCountingObject>(
    std::size_t count) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(
    GivenMap_WhenCreatedWithDefaultConstructor_ThenItIsEmpty,
    K,
    TestedKeyTypes) {
  const Map<K> map;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(
    GivenEmptyMap_WhenAddingItem_ThenItIsNoLongerEmpty,
    K,
    TestedKeyTypes) {
  Map<K> map;
                             ...
                             TestedKeyTypes)
{
 ...
 thenMapContainsItems(map, { { 1410, "Grunwald" }, { 753, "Rome" }, { 1789,
 "Paris" } }); thenMapContainsItems(other, { { 753, "Rome" }, { 1789, "Paris"
 } });
}
...
// If Iterator methods are to be changed, then new ConstIterator tests are
// required.
...
namespace std {
template <>
struct hash<OperationCountingObject> {
  size_t operator()(OperationCountingObject obj) const {
